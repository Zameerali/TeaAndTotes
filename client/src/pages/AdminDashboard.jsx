  ...
  return (
    <section className="py-16 bg-gray-100 dark:bg-gray-900 min-h-[80vh]">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-bold text-center mb-8 text-green-900 dark:text-white">Admin Dashboard</h2>
          ...
          <form onSubmit={handleAddProduct} className="space-y-6">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div {...getRootProps()} className="border-2 border-dashed border-green-300 dark:border-green-700 p-4 text-center cursor-pointer rounded-lg bg-gray-50 dark:bg-gray-900 hover:bg-green-50 dark:hover:bg-green-900 transition">
                <input {...getInputProps()} />
                <p className="text-gray-500 dark:text-white">{image ? image.name : 'Drag & drop or click to upload image'}</p>
              </div>
            </div>
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                onChange={handleProductChange}
                className="w-full border border-green-200 dark:border-green-700 rounded-lg px-4 py-2 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-400 transition"
                required
              >
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label className="text-sm font-medium text-gray-700 dark:text-white">Featured Product</label>
            </div>
            <button
              type="submit"
        ...
        {/* Product List */}
        <div className="mb-12">
          <h3 className="text-2xl font-semibold mb-4 text-green-900 dark:text-green-100">Manage Products</h3>
        ...
        {/* Order List */}
        <div>
          <h3 className="text-2xl font-semibold mb-4 text-green-900 dark:text-green-100">Manage Orders</h3>
          {orders.length === 0 ? (
              ...
              {orders.map((order) => (
                <div key={order._id} className="border rounded-2xl p-6 shadow-lg bg-white dark:bg-gray-800 animate-fade-in border-green-100 dark:border-gray-800">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2">
                    ...
                    <span className="text-sm text-gray-700 dark:text-gray-300">{order.userId.name} ({order.userId.email})</span>
                  </div>
                  <div className="mb-2 text-green-700 dark:text-green-300 font-medium">Total: PKR {order.total.toFixed(2)}</div>
                  {order.receipt && (
                    <div className="mb-2">
                      <span className="font-semibold text-gray-900 dark:text-gray-100">Receipt:</span>{' '}
                      <a
                        href={`/api/images/${typeof order.receipt === 'object' && order.receipt !== null ? order.receipt._id || order.receipt.id : order.receipt}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-green-600 dark:text-green-300 underline"
                      >
                        View Receipt
                      </a>
                    </div>
